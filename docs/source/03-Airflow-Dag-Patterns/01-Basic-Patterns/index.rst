...
这里还有一种特殊情况, 你需要传递的对象是一个有状态的 connection, 例如数据库 connection. 首先第一点, 不推荐在 task 之间传递 connection. 你应该在前一个 task 结束之后将 connection 断开. 然后在后一个 connection 中重新创建这个 connection. 因为原则上一个 Task 在外部看来应该是幂等的, 独立的, 无状态的. 你传递有状态的 connection 本身就破坏了 Airflow 的编程范式.
